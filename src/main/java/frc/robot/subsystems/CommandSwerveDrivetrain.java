...
public class CommandSwerveDrivetrain extends TunerSwerveDrivetrain implements Subsystem {
    ...
    private SwerveRequest.ApplyRobotSpeeds drive = new SwerveRequest.ApplyRobotSpeeds().withDriveRequestType(DriveRequestType.Velocity);
    ...
    String face = "6";
    public String pathstr = "6R";

    boolean doRejectUpdate;

    @SuppressWarnings({ "rawtypes", "unchecked" })
    private Odometry odo = new Odometry(this.getKinematics(), getPose().getRotation(), this.getState().ModulePositions,
            getPose());
    private PoseEstimator m_poseEstimator = new PoseEstimator<>(getKinematics(), odo, VecBuilder.fill(0.1, 0.1, 0.1),
    ...
    public Command followPathCommand6(String pathName) throws IOException, ParseException {
        try {
            PathPlannerPath path = PathPlannerPath.fromPathFile("6" + pathName);
    ...
    public Command followPathCommand8(String pathName) throws IOException, ParseException {
        try {
            PathPlannerPath path = PathPlannerPath.fromPathFile("8" + pathName);
    ...
    public Command AutoAlgn(String pathname) {
        ...
        return new InstantCommand(() -> System.out.println("nulled out"));
    ...
    public void periodic() {
        ...
        double tagarea;
        if (LimelightHelpers.getTV("limelight-high") && LimelightHelpers.getTA("limelight-high") > 0.4) {
            mt2 = LimelightHelpers.getBotPoseEstimate_wpiBlue("limelight-high");
        ...
        // double[] distPose =
        // LimelightHelpers.getCameraPose_TargetSpace("limelight-intake");

        doRejectUpdate = false;
        if (mt2 != null) {
            if (Math.abs(Math.toDegrees(drive.Speeds.omegaRadiansPerSecond)) > 720) // if our angular velocity is
        ...
        // SmartDashboard.putNumber("distance to tag", Math.sqrt(Math.abs(distPose[0] *
        // distPose[0] + distPose[1] * distPose[1])));
        ...
        // SmartDashboard.putBoolean("ll test",
        // LimelightHelpers.getTV("limelight-intake"));
