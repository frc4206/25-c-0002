...
/*----------Comments that begin with JP: were added by John Paul and not generated by CTRE */ 
public class CommandSwerveDrivetrain extends TunerSwerveDrivetrain implements Subsystem {
    //JP: Set up swerve simulation
    ...
    /*JP: Set up Network table publishers for odometry, Limelight, and Pose estimator field positions. 
     * All except the Pose estimator position is commented out becuase publishing data to network tables
     * uses a lot of CPU
    */
...
//   .getStructTopic("old pose Pub", Pose2d.struct).publish();
    ...
    //JP: Create odometry and Pose estmator objects
    //JP, TODO: Fix the rawtypes and unchecked warning
    ...
    public CommandSwerveDrivetrain(
        ...
        /*JP: This section was copied from the Path Planner documentation, however the get methods have been changed to use the Pose estimator for limelight integration
         * To revert this and only use odometry (dead reckoning), change the methods to the odometry methods
        */
    ...
    public void periodic() {
        ...
        //JP: Update Odometry estimator with current wheel positions
        odo.update(getPose().getRotation(), this.getState().ModulePositions);

        //------JP: Limelight Integration (based off of the inegeration on the Limelgiht website)-----

        //JP: Get the pose from network tables
        LimelightHelpers.PoseEstimate mt2 = LimelightHelpers.getBotPoseEstimate_wpiBlue_MegaTag2("limelight-intake");
        doRejectUpdate = false; //JP: varable for if the vision target data is bad
        if (mt2 != null) {
            ...
            if(mt2.tagCount == 0) //Check if there are no tags
            {
            doRejectUpdate = true;
            }
            if(!doRejectUpdate) //If the data passes the checks, use the data
            {
                Rotation2d rot = new Rotation2d(getPose().getRotation().getRadians()); //Convert to a Rotation2d object, reason for this is depricated
                //TODO: Delete rot and get the rotation directly from the odometry
                m_poseEstimator.update(rot, this.getState().ModulePositions); // update the pose estimator with current odometry measurements first
                
                //JP: Set the standard deviations of the Limelight, lower these numbers to trust the data more
                m_poseEstimator.setVisionMeasurementStdDevs(VecBuilder.fill(.7,.7,9999999)); 

                //JP: Add the vision measurement
                m_poseEstimator.addVisionMeasurement(
                    mt2.pose,
                    mt2.timestampSeconds);
            }else {
                //JP: if the tag data is bad update the pose normally with odometry
                m_poseEstimator.update(getPose().getRotation(), this.getState().ModulePositions);
                // System.out.println("updated without mt2");
            }

            //JP: Uncoment this to publish the limelight data to Network Tables
            // mt2Pub.set(mt2.pose);
        ...
        //JP: Uncoment these to publish the odometry data to Network tables
